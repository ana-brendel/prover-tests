...
(n: Nat)
(x y: Lst)
(H: lst_subset (lst_intersection x y) x)
(H1: lst_subset x (lst_intersection x y))
(H0: lst_mem n y = true)
(IHx: lst_subset x y -> lst_eq (lst_intersection x y) x)
: lst_subset (lst_intersection x y) (append (cons n nil) x).
Proof. Admitted.
